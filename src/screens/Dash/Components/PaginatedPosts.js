...
export default class PaginatedPosts extends PureComponent {
    constructor(props) {
        super(props);
                ...
                <View style={styles.builderListMainCont}>
                    ...
                    <ScrollView
                        ...
                        {/* <FlatList
                            ...
                            showsHorizontalScrollIndicator={false}
                            horizontal={true}
                            contentContainerStyle={{ paddingRight: Platform.OS == "android" ? 100 : 0 }}
                            scrollEnabled={true}
    ...
    componentDidMount = () => {
        setImmediate(() => {
            this.setState({
                ...
                dataShow: this.props.data[0] == undefined ? [] : this.props.data[0][this.state.pageselected]
            })
        })
    }

    changePage = (page, index) => {
        var { dataShow, pageselected } = this.state
        var { data } = this.props

        dataShow = data[index][page + '']
        console.log("datashow", data[index]);
        setImmediate(() => {
            this.setState({
                dataShow,
                pageselected: page
    ...
    render() {
        const { data, refreshKey, allowBuilders, showBuilderList, builderData, paginationArr } =
            this.props;
        return (
            <View style={styles.mainContainer}>
                ...
                {paginationArr.length == 0 ? <></> : <ScrollView >
                    <View style={{
                        // width: width - 20,
                        height: 50,
                    ...
                    }}>
                        {
                            paginationArr.map((item, index) => {
                                // console.log("item pagination map", item);
                                return (
                                    <TouchableOpacity
                                        onPress={() => this.changePage(parseInt(item) + 1, index)}
                                        style={{
                                            padding: 10,
                                            marginHorizontal: 5,
                                            borderRadius: 15,
                                            backgroundColor: this.state.pageselected == parseInt(item) + 1 ? Colors.light_blue : Colors.white
                                        }}>
                                        <Text style={{
                                            fontSize: 16,
                                            fontWeight: "600",
                                            color: this.state.pageselected == parseInt(item) + 1 ? Colors.white : Colors.light_blue
                                        }}>{`${parseInt(item) + 1}`}</Text>
                                    </TouchableOpacity>
                                )
                            })
                        }
